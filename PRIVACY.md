...
The real editor test observes no non-GET/HEAD requests during synthetic import, scene scan, project edits and MP4 export. Whisper/MiniLM browser tests use actual remote weight downloads and local inference. These checks are not a general audit of third-party code, browser extensions, or provider infrastructure. After human license acceptance, a separate Clear/Uhm runtime smoke completed and observed one 211-byte SDK usage POST to https://platform.desertant.ai/api/v1/ingest (top-level metadata fields: app, events, platform, sdk, sentAt). No media upload request was observed. This short test is not a comprehensive audit of every SDK reporting path.
